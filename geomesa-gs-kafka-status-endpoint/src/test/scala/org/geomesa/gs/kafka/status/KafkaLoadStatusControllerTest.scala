  ...
  "KafkaLoadStatusController" should {
    ...
    "return error if stores are not loaded" in {
      ...
      def getStore[T](ignored: Any): T = {
        KafkaCacheLoader.LoaderStatus.startLoad(this); null.asInstanceOf[T]
      }
      kafkaInfo.getDataStore(null) answers getStore _
      val otherInfo = mock[DataStoreInfo]
      otherInfo.getConnectionParameters returns Collections.singletonMap(KafkaDataStoreParams.Brokers.key, "localhost")
      val controller = new KafkaLoadStatusController()
      val catalog = mock[Catalog]
      ...
      KafkaCacheLoader.LoaderStatus.completedLoad(this)
